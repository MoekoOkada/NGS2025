...
showSigOfNodes(BP, score(test), useInfo = "def")
```
...
BP_Table <- BP_Table %>% slice_min(weightFisher, n = 20)
BP_Table$Term <- str_wrap(BP_Table$Term, width = 45)
head(BP_Table)

p <- ggplot(BP_Table, aes(x = order(Term, as.numeric(BP_Table$weightFisher)), y = -log10(as.numeric(weightFisher)))) +
