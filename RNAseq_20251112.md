...
```R
# Comp.txtの読み込み
df <- read.table("Comp.txt", header = T, row.names = NULL, sep = "\t")

# 列名が無い場合の補正
colnames(df) <- c("ID", "logFC", "logCPM", "LR", "PValue", "FDR")
...
df2 <- df %>% filter(ID %in% valid)

# 同一遺伝子に複数トランスクリプトがある場合、最小FDRで代表
df2 <- df2 %>% arrange(ID, FDR) %>% distinct(ID, .keep_all = TRUE)

# FDR < 0.01 かつ、２倍以上変動（|logFC| > 1）をDEGとする
df2 <- df2 %>% mutate(isDEG = FDR <= 0.01 & abs(logFC) > 1)
```
...
```R
gene_universe <- factor(as.integer(df2$isDEG))
...
```R
# gene2GO を org.At.tair.db から作る
make_gene2go <- function(tairs) {
  gotab <- AnnotationDbi::select(org.At.tair.db,
                                 keys = unique(tairs),
                                 keytype = "TAIR",
                                 columns = c("GO", "ONTOLOGY"))
  gotab <- gotab[!is.na(gotab$GO), ]
  # gene2GO: 名前=TAIR, 値=その遺伝子に紐づくGO IDのベクトル
  split(gotab$GO, gotab$TAIR)
}
gene2GO <- make_gene2go(names(gene_universe))
```
...
BP <- new("topGOdata",
            ...
            gene2GO = gene2GO)
test <- runTest(BP, algorithm = "elim", statistic = "fisher")

BP_Table <- GenTable(BP,
  weightFisher = test,
  orderBy = "classicFisher", ranksOf = "classicFisher", topNodes = sum(score(test) < .05)
)

# output of detail results
write.table(BP_Table, file = "topGO_BP_table.txt", sep = "\t", quote = F, col.names = T, row.names = F)

# ネットワーク図作成
showSigOfNodes(BP, test, useInfo = "def")

# 相性が悪く、ネットワーク図の作図でエラーが出る場合は
# RgraphvizパッケージをBiocManagerからインストールし、下記を実行
png("GO_BP_network.png", width = 800, height = 600)
par(cex = 0.5)
plotGraph(BP, test, 
           firstSigNodes = 20, 
           useInfo = "def", 
...
棒グラフ（barplot）はカテゴリ名とp値もしくはFDRの関係を示す。

今回は上位20項目を抽出して、棒グラフを作成する。
...
#### 上位20のGO項目を抽出して棒グラフを書く

```R
# データの調整
BP_for_plot <- BP_Table %>%
  mutate(
    p = (as.numeric(weightFisher))) %>% # 文字列を数値に変更し、pという新しい列に追加
  filter(!is.na(p)) %>% # データ欠損している項目を除去
  slice_min(p, n = 20) %>% # 上位20件を抽出
  arrange(p) %>% #　FDRが小さい方からp列を並び替える
  mutate(Term = factor(Term, levels = rev(Term))) # 並び替えた列を固定する

# X軸にGO term、Y軸に-log10(p)（小さい順に並べ替えたFDR値の-log10）をとってプロット
plot1 <- ggplot(BP_for_plot, aes(x = Term, y = -log10(p))) +
  geom_col(fill = "steelblue") +
  coord_flip() + # グラフを90°傾ける
