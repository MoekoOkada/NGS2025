...
src $ wget -O genome.tar.gz https://x.gd/TkHz4V
src $ ls
...
src $ perl scripts/1-1_ModifyQCresults_PE.pl 1_qc/20251111_results_qc.txt 1_qc/20251111_qc_results_mod.txt
src $
```
...
data <- data[, -8]
data <- select(data, -Sample.name)
head(data)

# Unmapped reads
total <- as.numeric(data[1, ]) * 2
Pair <- as.numeric(data[2, ]) * 2
Mult <- as.numeric(data[3, ]) * 2 + as.numeric(data[7, ])
Disc <- as.numeric(data[4, ]) * 2
Unmap <- as.numeric(data[5, ])
single <- as.numeric(data[6, ]) * 2
data3 <- rbind(Pair, Mult, Disc, single, Unmap)
colnames(data3) <- colnames(data)
head(data3)

ID <- c("Total", "Pair", "Multimapped", "discordantly", "single", "Unmapped")
data4 <- cbind(sample = ID, data3)

write.table(data4, file = "hisat_results_mod_2.txt", col.names = TRUE, sep = "\t", quote = FALSE, row.names = FALSE)
